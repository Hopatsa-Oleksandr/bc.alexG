(function () {
  ...
  popup.addEventListener("submit", function (event) {
    if (!login.value || !password.value) {
      ...
      setTimeout(function () {
        popup.classList.add("enter-form-error");
      }, 1);
    } else {
  ...
  window.addEventListener("keydown", function (event) {
    if (event.keyCode == 27 && popup.classList.contains("enter-form-show")) {
  ...
  document.addEventListener("click", function (e) {
    var formOpened = document
      .querySelector(".enter-form")
      .classList.contains("enter-form-show");
    if (
      formOpened == true &&
      !(e.target == link) &&
      !(e.target == popup) &&
      !(e.target.parentNode == popup) &&
      !(e.target.parentNode.parentNode == popup)
    ) {
      popup.classList.remove("enter-form-show");
      popup.classList.remove("enter-form-animation");
    }
  ...
  if (!(container === null)) {
    (btn = container.querySelectorAll(".btn")),
      (photos = Array.prototype.slice.call(
        document.querySelectorAll(".int-photo")
      )),
    ...
    for (i = 0; i < btn.length; i++) {
      btn[i].addEventListener("click", function (event) {
        ...
        if (
          this.className === "btn next" &&
          indexOfVisible < photos.length - 1
        ) {
